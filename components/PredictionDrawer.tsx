...
/** ===== Local minimal types to avoid '@/lib/types' import ===== */
type Game = {
  gameId: string;
  league: string;
  homeTeam: string;
  awayTeam: string;
  /** ISO string, number, or Date accepted by new Date(...) */
  time: string | number | Date;
};

type PickSummary = {
  winner: string; // use your enum/string union if you have one
  confidence: number; // 0..1
  reasons?: string[];
};

type BaseAgentExecution = {
  name: string;
  output?: unknown;
...
type AgentExecution = BaseAgentExecution & {
  weight?: number;
  description?: string;
  scoreTotal?: number;
...
const PredictionDrawer: React.FC<Props> = ({ game, isOpen, onClose }) => {
  ...
  const startRun = () => {
    ...
    const liveMode =
      typeof window !== "undefined"
        ? localStorage.getItem("LIVE_MODE") ?? "off"
        : "off";
  ...
  useEffect(() => {
    ...
    if (data.type === "agent") {
      setExecutions((prev) => [...prev, data as AgentExecution]);
      if (typeof data.confidenceEstimate === "number") {
        setConfidence(Math.round(data.confidenceEstimate * 100));
      }
    } else if (data.type === "lifecycle") {
      handleLifecycleEvent(data);
    } else if (data.type === "summary") {
      const nextPick = data.pick as PickSummary;
      setPick(nextPick);
      setConfidence(Math.round(nextPick.confidence * 100));
      void logUiEvent("landingPredictionComplete", {
        winner: nextPick.winner,
        confidence: nextPick.confidence,
      });
    } else if (data.type === "error") {
      void logUiEvent("landingPredictionError", { code: data.code });
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [lastMessage]);
  ...
  const mockReasons = [
    {
      agent: "InjuryScout",
      weight: 0.4,
      explanation: "Key player injury on the opposing team.",
    },
    {
      agent: "LineWatcher",
      weight: 0.35,
      explanation: "Significant line movement favoring the home team.",
    },
    {
      agent: "StatCruncher",
      weight: 0.25,
      explanation: "Historical performance trends support this pick.",
    },
  ];
  ...
  return (
    ...
    >
      ...
      <div
        ref={drawerRef}
        className="relative w-full max-w-md h-full bg-slate-900 text-white flex flex-col rounded-xl overflow-hidden"
      >
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          {/* Live agent run (SSE) */}
          <AgentRunPanel matchupId={game.gameId} />

          <div className="mt-6">
            <ExplainPick reasons={mockReasons} />
          </div>
